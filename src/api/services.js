...
export async function updatePost(postId, { title, content, published, image_url, image_public_id }) {
...
export async function updateUser(userId, { email, password }) {
  const { data } = await api.put(`/users/${userId}`, { email, password });
  return data;
}
...
export async function followUser(followingId) {
  const { data } = await api.post('/follow', { following_id: followingId });
  return data;
}
...
export async function getFollowers(userId, limit = 20, skip = 0) {
  const { data } = await api.get(`/users/${userId}/followers`, { params: { limit, skip } });
  return data;
}

export async function getFollowing(userId, limit = 20, skip = 0) {
  const { data } = await api.get(`/users/${userId}/following`, { params: { limit, skip } });
  return data;
}
...
export async function fetchUserPosts(userId, maxPosts = 50) {
  ...
  while (userPosts.length < maxPosts) {
    const { data } = await api.get('/posts', { params: { limit, skip } });
    userPosts.push(...data.filter((item) => item.post.owner_id === userId));
...
export async function fetchSavedPosts(limit = 20, skip = 0) {
  const { data } = await api.get('/saved', { params: { limit, skip } });
